...
def convert_linear_layer_to_lora(model,
    ...
    for name, module in model.named_modules():
        if isinstance(module, nn.Linear):
            for part_module_name in part_module_names:
                if part_module_name in name:
                    print('Replace_LoRA_Module: ', name)
...
def only_optimize_lora_parameters(model, force_optimize_params=[]):
    ...
    for name, param in model.named_parameters():
        if "selector" in name or "lora_right_weight" in name or "lora_left_weight" in name or name in force_optimize_params:
            print('grad:', name)

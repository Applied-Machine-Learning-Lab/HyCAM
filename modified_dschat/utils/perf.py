...
def print_throughput_simple(hf_model, args, e2e_time, rank=0, estimate_remaining_time=float('inf')):
    if rank <= 0:
        ...
        print(
            f"Model Parameters: {param_string}, Latency: {e2e_time:.2f}s, Samples/sec: {samples_per_second:.2f}, Time/seq {e2e_time/batch_size:.2f}s, Batch Size: {batch_size}, Sequence Length: {seq_length}, Remaining: {estimate_remaining_time//60}min"
        )

def print_throughput(hf_model, args, e2e_time, rank=0, estimate_remaining_time=float('inf')):
    if rank <= 0:
        ...
        param_string = f"{params_in_billions:.3f} B" if params_in_billions != 0 else "NA"
        print(
            f"Model Parameters: {param_string}, Latency: {e2e_time:.2f}s, TFLOPs: {train_tflops:.2f}, Samples/sec: {samples_per_second:.2f}, Time/seq {e2e_time/batch_size:.2f}s, Batch Size: {batch_size}, Sequence Length: {seq_length}, Remaining: {estimate_remaining_time//60}min"
        )

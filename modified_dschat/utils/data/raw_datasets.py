...
# English dataset
class AlpacaCotDataset(PromptRawDataset):

    def __init__(self, output_path, seed, local_rank, dataset_name, select_num):
        super().__init__(output_path, seed, local_rank, dataset_name)
        self.dataset_name = dataset_name
        self.dataset_name_clean = '_'.join(dataset_name.strip('/data/ca8442c35eb4400e8b9cc7e8637d7c11/datasets').strip('.json').split('/'))
        if select_num:
            print('!Dataset Selected: ', select_num)
            self.raw_datasets["train"] = self.raw_datasets["train"].select(range(select_num))
            print('!Curr Data ALL SUM: ', len(self.raw_datasets["train"]))
    def get_train_data(self):

        
        from .data_utils import get_raw_dataset_split_index
        dataset = self.raw_datasets["train"]
        print('!get_train_data ALL SUM: ', len(dataset))
        index = get_raw_dataset_split_index(self.local_rank, self.output_path,
                                            self.dataset_name_clean,
                                            self.seed, "train_eval", "7,2", 0,
                                            len(dataset))
        dataset = Subset(dataset, index)

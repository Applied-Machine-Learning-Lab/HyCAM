...
def get_tokenizer(model_name_or_path, fast_tokenizer=True):
    tokenizer = AutoTokenizer.from_pretrained(
        model_name_or_path, device_map='auto', padding_side='left', fast_tokenizer=fast_tokenizer)
...
def get_optimizer_grouped_parameters(
    ...
    lora_name_list=["lora_right_weight", "lora_left_weight", 'u_trans'],
):
    ...
    print('optimizer_grouped_names', optimizer_grouped_names)
    non_empty_groups = []
...
def save_full_model(model_engine, tokenizer, global_rank, save_dir, tag):
    CONFIG_NAME = "config.json"
    WEIGHTS_NAME = "pytorch_model.bin"
    
    final_save = os.path.join(save_dir, tag)
    os.makedirs(final_save, exist_ok=True)
    
    model_engine.module.eval()

    with deepspeed.zero.GatheredParameters(model_engine.module.parameters(), modifier_rank=None):
        state_dict = model_engine.module.state_dict()

    output_model_file = os.path.join(final_save, WEIGHTS_NAME)
    output_config_file = os.path.join(final_save, CONFIG_NAME)

    if global_rank == 0:
        torch.save(state_dict, output_model_file)
        print('Save Full Model to', output_model_file)
